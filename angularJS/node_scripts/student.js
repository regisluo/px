        ...
        if (data) {
            ...
            fs.readFile(dataFile, function (error, content) {
                if (error) {
                    ...
                    data.id = nextId;
                    students.push(data);
                    console.log("current size: " + students.length);
...
exports.deleteStudent = function (request, response, data) {
    try {
        if (data) {
            var students = null;
            fs.readFile(dataFile, function (error, content) {
            ...
            msg.sendJson(request, response, JSON.stringify({ "status": "200", "data":"success" }));
        } else {
            throw new Error("Request data not valid.");
        }
    } catch (error) {
